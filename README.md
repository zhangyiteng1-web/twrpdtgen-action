...
## 目录结构（生成结果示例）
TWRP设备树生成Action
我们收到了用户请求：输出更新后的README，要体现自动创建仓库和标准化的推送规则。需要输出完整README，在代码块中。

我们需要在README中更新“配置独立仓库推送”部分，加入自动创建仓库的说明，以及repo_name自动生成规则（不填则按标准生成）。还要更新参数表格，说明repo_name可选，若不填写将自动生成。
